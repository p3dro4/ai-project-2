          ...
          (t (let* ((jogador-necessita-troca (cond ((equal jogador *cavalo-branco*) nil) (t t)))
                ...
                (cond ((null (first jogada)) 
                      ...
                      (t (with-open-file (ficheiro (log-dat) :direction :output :if-exists :append :if-does-not-exist :create)
                            ...
                            (format t "nos expandidos: ~a~%" (fourth jogada))
                            (format t "nos gerados: ~a~%" (fifth jogada))
                            (format t "nos cortados: ~a~%" (sixth jogada))
                            (format t "tempo de execucao: ~3,1f ms~%~%" (* (third jogada) 1000.0))
                            (format ficheiro "~46,1,1,'.:@< jogada - jogador ~a ~>~%" (cond ((equal jogador *cavalo-branco*) "1") ((equal jogador *cavalo-preto*) "2")))
                            (format ficheiro "~46,1,1,'#:@< computador ~>~%")
                            (escreve-tabuleiro-formatado (first estado-final) ficheiro t t 1)
                            (format ficheiro "~46,1,1,'#:@<~>~%")
                            (format ficheiro "utilidade: ~a~%" (second jogada))
                            (format ficheiro "nos expandidos: ~a~%" (fourth jogada))
                            (format ficheiro "nos gerados: ~a~%" (fifth jogada))
                            (format ficheiro "nos cortados: ~a~%" (sixth jogada))
                            (format ficheiro "tempo de execucao: ~3,1f ms~%~%" (* (third jogada) 1000.0))
                            (close ficheiro)
                            estado-final
                          )
                      )
