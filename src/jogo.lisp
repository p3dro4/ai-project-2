...
(defun colocar-cavalo (tabuleiro cavalo)
  "Coloca o cavalo na casa com maior valor na linha respetiva"
  (cond ((not (numberp cavalo)) (error "O cavalo tem de ser um numero"))
        ...
        (t (cond ((= cavalo -1) (colocar-cavalo-auxiliar tabuleiro cavalo 0))
                 ((= cavalo -2) (colocar-cavalo-auxiliar tabuleiro cavalo (1- (length tabuleiro))))
           )
        )
  )
)
...
(defun colocar-cavalo-auxiliar (tabuleiro cavalo linha-respetiva)
  "Coloca o cavalo na casa com maior valor na linha respetiva"
  (let* ((valor (maior-numero-linha (linha linha-respetiva tabuleiro)))
    ...
    (cond ((null (celula linha-respetiva coluna tabuleiro)) nil)
          (t (aplicar-regras (substituir linha-respetiva coluna tabuleiro cavalo) (celula linha-respetiva coluna tabuleiro)))
    )
  )
)
...
(defun numeros-tabuleiro (tabuleiro)
  (remove-if (lambda (num) (or (< num 0) (null num))) (juntar-linhas tabuleiro))
)
...
(defun posicao-cavalo (tabuleiro cavalo)
...
(defun cavalo-colocado-p (tabuleiro cavalo)
  "Verifica se o cavalo está colocado no tabuleiro"
  (cond ((posicao-cavalo tabuleiro cavalo) t)
        (t nil)
  )
)
